...
rownames(stats)<- "Multiple linear Regression"
...
statsDF<- rbind(stats,loocvDataFrame,kDataFrame,repeatedkPredictkDataFrame) 
statsDF

## 4. Fit KNN regression, Decision Tree regression, SVM regression and Neural Network regression using the same dependent and independent variables, get and compare fit indices of these models for SR.test data KNN regression
...
NNmodel<- neuralnet(bloodPressure ~., data= NNTraindata, hidden= c(15,14,13),stepmax =99999, linear.output= FALSE, threshold = 0.001 )
NNpredict<- compute(NNmodel,NNTestdata)
#NNpredict$net.result*(max(SR.test$bloodPressure)-min(SR.test$bloodPressure)) + min(SR.test$bloodPressure)
NNpredict$net.result

# deviation<- (NNTestdata$bloodPressure- NNpredict$net.result)/NNTestdata$bloodPressure
# accuracy<- abs(mean(deviation))
# error<- 1- accuracy

nnR2<- R2(NNpredict$net.result, NNTestdata$bloodPressure)
nnMSE<- mean((NNpredict$net.result - NNTestdata$bloodPressure)^2)
nnRMSE<- RMSE(NNpredict$net.result, NNTestdata$bloodPressure)
nnMAE<- MAE(NNpredict$net.result, NNTestdata$bloodPressure)
nnR2
nnMSE
nnRMSE
nnMAE
...
fitDataFrame<- data.frame(R2 =c(KNNr2,svmR2,nnR2),MSE=c(KNNmse,svmMSE,nnMSE),RMSE=c(KNNrmse,svmRMSE,nnRMSE),MAE=c(KNNmae,svmMAE,nnMAE))
rownames(fitDataFrame)<- c("KNN","SVM","NN")
fitDataFrame
...
predictDataFrame<- data.frame(50,175,80,90000)
a<- c(1,1,0,1,0,0,0,0,1,0)

normalisingTheData<- (predictDataFrame - min(SR.test[,1:4]))/(max(SR.test[,1:4])-min(SR.test[,1:4])) 
predictDataFrame<- cbind(normalisingTheData,data.frame(t(a)))
fidBloodPressure<- compute(NNmodel,predictDataFrame)
fidBloodPressure$net.result*(max(SR.test$bloodPressure)-min(SR.test$bloodPressure)) + min(SR.test$bloodPressure)
